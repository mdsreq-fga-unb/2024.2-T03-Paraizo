...
export default function DocFichaRpg({patient}:InterfaceDocFichaRPGComponent){
    ...
    useEffect(()=> {
        if(patientDoc.profissao === '' && patientDoc.etiologia === '' && patientDoc.alongamentos === '')
            setDocStatus(false)
    }, [])
    ...
    return <>
    {
        ...
        <section>
            <NotFoundDoc
            title="Este paciente não possui uma Ficha de Avaliação - RPG registrada!"
            text="Deseja preencher uma ficha de Ficha de Avaliação - RPG para este paciente?"/>
        </section>
    }
    </>
}